...
module.exports = function(config) {
  ...
  if (config.main) {
    ...
    if (fs.existsSync(config.main) === false) {
      config.main = absolutePath(config.main.replace('.ts', '.tsx'));
    }
  }

  config.tmpMain = path.resolve(config.tmp + '/src/' + (path.basename(config.main).replace('.tsx', '.js').replace('.ts', '.js')));
  config.libMain = path.resolve(config.lib + '/' + (path.basename(config.main).replace('.tsx', '.js').replace('.ts', '.js')));
