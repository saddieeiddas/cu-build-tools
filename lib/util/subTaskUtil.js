 ...
 */
export function resolveComponentDirectory(root, component, callback) {
...
function resolveTask() {
  ...
  if (argv._.length > 0) {
    ...
    if (parts.length >= 2 && parts[1] !== '') {
      const directory = resolveComponentDirectory(parts[0]);
      if (directory) {
        return {
          'component': parts[0],
          'task': parts[1],
 ...
 */
export function findComponentDirectories(root, callback) {
  ...
  globby(globs, (err, paths) => {
    if (err || paths.length === 0) {
      ...
      callback(false, paths.map((p) =>  path.dirname(p)));
    }
  });
}
...
export function executeTask(gulp, options, cb) {
  resolveComponentDirectory(options.path, options.component, (err, componentDirectory) => {
    if (err) {
      util.log(util.colors.red(err));
      cb();
    } else {
      gulp.src(componentDirectory + '/gulpfile.js', {'read': false})
        .pipe(chug({
          'tasks': [options.task],
...
export function executeTaskOnAllComponents(gulp, options, cb) {
  findComponentDirectories(options.path, (err, components) => {
    if (err || components.length === 0) {
      util.log(util.colors.red(err));
      cb();
    } else {
      const gulps = components.map((componentDirectory) => componentDirectory + '/gulpfile.babel.js');
