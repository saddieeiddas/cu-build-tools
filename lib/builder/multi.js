...
module.exports = function (gulp, options) {
  ...
  gulp.task('build', function(cb) {
    getDirectories(function(directories) {
      if (directories.length > 0) {
        executeNextSubTask(directories, 'build', cb);
      } else {
        cb();
      }
    });
  ...
  gulp.task('debug:legacy', function(cb) {
    getLegacyDirectories(function(directories) {
      plugins.util.log(directories);
      cb();
    });
  })
  ...
  gulp.task('compile:legacy', function(cb) {
    getLegacyDirectories(function(directories) {
      ...
      if (directories.length > 0) {
        ...
        return result.js.pipe(plugins.sourcemaps.write({sourceRoot: '../',includeContent: true})).pipe(gulp.dest(config.dir.publish));
      } else {
        cb();
      }
    });
  ...
  function executeNextSubTask(directories, task, cb) {
    if (directories.length <= 0) {
      cb();
    } else {
      ...
      executeSubTask(taskConfig, task, function() {
        executeNextSubTask(directories, task, cb);
      });
    }
  }
